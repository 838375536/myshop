...
public class TradeServiceImpl implements TradeService {
        ...
        for(ShopcarGood sg:shopcarGood){
            ...
            for(Good good: goods){
                if(sg.getGoodId()==good.getId()){
                    if(other!=null&&other.trim()!="") {             // 2018-5-15
    ...
    //没开发完成
    @Override
    public List<TradeReport> selectTodayTradeBySellerId(Integer sellerId) {
        BigDecimal maxPay = new BigDecimal(0);//成交最大额度
        BigDecimal amount = new BigDecimal(0);//成交总额
        int payCount =0; //成交数量
        int unpayTradeCount =0; // 待付款订单数量
        int unsendTradeCount =0;//代发货订单数量
        int failTradeCount =0;//失败订单数量
       List<Trade> trades =  tradeMapper.selectTodayTrade();
    ...
    @Override
    public List<TradeReport> selectMonthTradeBySellerId(Integer sellerId) {
        return null;
    }

    @Override
    public List<TradeReport> selectYearTradeBySellerId(Integer sellerId) {
        return null;
    }
}
